...
def hysteresis_test(pitchrollN, spacing, pitchroll, time_period, norm_exp_data, offset_gnd_dat):
    ...
    if pitchroll == "pitch":
        ...
        norm_exp_data = norm_exp_data[time_offset:]
    ...
    for i in range(0, len(norm_exp_data), window_len):
        ...
        check+=1
    t_load = np.linspace(0, window_len/FPS, loading_df.shape[1])
    t_unload = np.linspace(0, window_len/FPS, unloading_df.shape[1])
